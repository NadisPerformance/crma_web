...
class Edit extends React.Component {
  constructor (props) {
    ...
    this.fariane= [{title:"Acceuil",path:"/"},{title:"Locations",path:"/insurances/list"}]
        ...
        <Page title="Locations" fariane={this.fariane}>
            <Query query={get_insurance_to_update} variables={{insuranceId}} _pollInterval={3000} >
              {({ loading, error, data }) => {
                ...
                insurance.customerId = this.state.insurance.customerId * 1
                insurance.carId = this.state.insurance.carId * 1
                insurance.second_driverId = this.state.insurance.second_driverId *  1
                delete insurance.customer
                delete insurance.car
                //console.log(user)
                return (
